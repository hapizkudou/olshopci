...
class User extends MY_Controller {
    ...
    public function create()
    {
        if(! $_POST){
            ...
            $this->load->library('form_validation'); //baris ini untuk membuat rules password, karna di model password tidak di set
    ...
    public function edit($id)
    {
        ...
        if(! $this->user->validate()){
            $data['title']          = 'Admin: Ubah Pengguna';
    ...
    {
        $images = glob("uploads/user/*.*");
        $jumlah = false;
        for ($i=0; $i<count($images); $i++)
        {
            if(! $this->user->where('image', substr($images[$i], 13))->first() && substr($images[$i], 13) != 'default.png')
            {
                unlink($images[$i]);
                $jumlah = true;
            }
